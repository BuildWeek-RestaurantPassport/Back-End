...
router.get("/passport", (req, res) => {
  let { id } = req.decodedJwt;
  newRest.getRestaurantsByUserId(Number(id)).then(results => {
    res.json(results);
  });
});
...
router.delete("/:id", (req, res) => {
  ...
  newRest
    ...
    .then(results => {
      ...
      if (results) {
        res.json({
          message: "Hope you didnt need that now it is gone forever!."
        });
      } else {
